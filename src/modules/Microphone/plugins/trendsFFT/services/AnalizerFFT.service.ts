...
import { UserPattern } from "../types/patterns";
...
export class AudioFileAnalyzerService {
    ...
    async analyzeAudioFileWithReport(
        ...
        config,
        rawFrames,
        normalizedResult.frames,
        normalizedResult.statistics,
        normalizedResult.temporalPatterns,
        recommendations,
        normalizedResult.duration,
        normalizedResult.sampleRate
        );
  ...
  private calculateNormalizedTemporalPatterns(frames: any[], duration: number): any {
    ...
    // Частотные скачки - создаем объект с правильной структурой
    const jumps: number[] = [];
  ...
  generatePatternRecommendations(analysisResult: AudioAnalysisResult): Partial<UserPattern> {
    ...
    return {
      ...
      temporalPatterns: {
        ...
        frequencyJumps: {
          ...
          densityPerSecond: {
            max: Math.min(20, temporalPatterns.frequencyJumps.densityPerSecond * 1.5)
          }
        },
